-- 021_cfdi_40.sql
-- 1. Asegurar tipo y constraint para tipo_comprobante en factura
ALTER TABLE factura ALTER COLUMN tipo_comprobante TYPE CHAR(1);
ALTER TABLE factura ADD CONSTRAINT chk_tipo_comprobante CHECK (tipo_comprobante IN ('I', 'E'));

-- Asegurar tipo para serie
ALTER TABLE factura ALTER COLUMN serie TYPE VARCHAR(10);

-- 2. Crear tabla cfdi_relacionado
CREATE TABLE IF NOT EXISTS cfdi_relacionado (
    ...
    CONSTRAINT fk_cfdi_relacionado_factura FOREIGN KEY (id_factura) REFERENCES factura(id_factura) ON DELETE CASCADE
);

-- 3. Modificar fn_crear_venta para recibir p_folio, p_serie, p_tipo_comprobante
CREATE OR REPLACE FUNCTION public.fn_crear_venta(p_id_cliente integer, p_rfc_receptor character varying, p_nombre_receptor character varying, p_cp_receptor character varying, p_regimen_receptor character varying, p_uso_cfdi character varying, p_id_forma_pago integer, p_id_metodo_pago integer, p_moneda character varying, p_tipo_cambio numeric, p_subtotal numeric, p_total_impuestos_trasladados numeric, p_total_impuestos_retenidos numeric, p_total numeric, p_observaciones text, p_conceptos_json jsonb, p_id_vendedor integer DEFAULT NULL::integer, p_folio integer DEFAULT NULL::integer, p_serie character varying DEFAULT 'A'::character varying, p_tipo_comprobante character varying DEFAULT 'I'::character varying)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF p_id_vendedor IS NOT NULL THEN
        v_comision := ROUND((p_subtotal * 0.02), 2);
    ELSE
        v_comision := 0;
    END IF;
    ...
    INSERT INTO factura (
        ...
        id_vendedor, comision_vendedor,
        tipo_comprobante
    ) VALUES (
        p_id_cliente, p_serie, p_folio, NOW(), 
    ...
    -- Registrar Pago automático si es de Contado (PUE)
    IF v_metodo_clave = 'PUE' THEN
        DECLARE
