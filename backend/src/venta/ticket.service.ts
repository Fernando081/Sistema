...
import { DatosTicketFactura } from './ticket.types';
...
function numeroALetras(amount: number, moneda: string): string {
  ...
  function convertGroup(n: number): string {
    ...
    } else if (n >= 20 && n <= 29) output += veinti[n - 20];
    else if (n >= 10 && n <= 19) output += diez[n - 10];
  ...
  if (millones > 0)
    texto +=
      millones === 1 ? 'UN MILLON ' : convertGroup(millones) + 'MILLONES ';
...
export class TicketService {
  async crearPdfFactura(datos: DatosTicketFactura): Promise<Buffer> {
    // --- PREPARACIÓN DE DATOS ---

    // 1. Total con Letra
    const textoImporte = numeroALetras(
      Number(datos.total),
      datos.moneda || 'M.N.',
    );
    ...
    const fechaEmisionStr = fechaEmision
      .toISOString()
      .replace('T', ' ')
      .substring(0, 19);
    ...
    const direccionCliente = [
      ...
      .filter(Boolean)
      .join(', '); // Une con comas y elimina vacíos
        ...
        // ==========================================
        {
          columns: [
            { width: 120, text: '' }, // Espacio para Logo
            {
              ...
              stack: [
                {
                  text: 'RODRIGO DANIEL ORTEGA CORREA',
                  fontSize: 11,
                  bold: true,
                },
                {
                  text: 'REFACCIONES Y TRACTOREPUESTOS',
                  fontSize: 10,
                  bold: true,
                  italics: true,
                  margin: [0, 2],
                ...
                {
                  text: 'RÉGIMEN FISCAL: 626 - SIMPLIFICADO DE CONFIANZA',
                  fontSize: 7,
                  bold: true,
                },
                {
                  text: 'SOR JUANA INES DE LA CRUZ 201-A, COL. NUEVO PROGRESO',
                  fontSize: 7,
                },
              ...
              stack: [
                ...
                { text: 'CFDI', fontSize: 8, bold: true, alignment: 'center' },
                {
                  ...
                  table: {
                    ...
                    body: [
                      [
                        { text: 'FACTURA', bold: true, fontSize: 7 },
                        {
                          text: `${datos.serie || ''} ${datos.folio}`,
                          alignment: 'right',
                          fontSize: 7,
                          bold: true,
                        },
                      ...
                      [
                        { text: 'FECHA EMISIÓN:', bold: true, fontSize: 6 },
                        {
                          text: fechaEmisionStr,
                          alignment: 'right',
                          fontSize: 6,
                        },
                      ],
                      [
                        { text: 'FECHA CERTIF:', bold: true, fontSize: 6 },
                        { text: fechaCertStr, alignment: 'right', fontSize: 6 },
                      ],
                      [
                        {
                          text: 'CERTIFICADO EMISOR:',
                          bold: true,
                          fontSize: 6,
                          colSpan: 2,
                        },
          ...
          table: {
            ...
            body: [
              [
                ...
                {
                  text: `Tampico, Tamaulipas - CP: 89605`,
                  fontSize: 7,
                  fillColor: '#eee',
                },
              ],
          ...
          table: {
            ...
            body: [
              ...
              ...datos.conceptos.map((p: any, i: number) => [
                ...
                {
                  text: Number(p.precio).toFixed(2),
                  fontSize: 7,
                  alignment: 'right',
                },
                {
                  text: Number(p.importe).toFixed(2),
                  fontSize: 7,
                  alignment: 'right',
                },
              ]),
        ...
        if (currentPage === pageCount) {
          return {
            ...
            stack: [
                ...
                columns: [
                  // Lado Izquierdo: Letra, Datos Pago, QR, Sellos
                  {
                    ...
                    stack: [
                      ...
                      {
                        text: `Forma de Pago: ${datos.forma_pago}`,
                        fontSize: 7,
                      },
                      {
                        text: `Método de Pago: ${datos.metodo_pago}`,
                        fontSize: 7,
                      },
                        ...
                        columns: [
                          { qr: qrData, fit: 70, width: 80 },
                          {
                            ...
                            stack: [
                              ...
                              {
                                text: 'kJS8s7d89s7d98s7d98s7d98s7d9s8d7s9d87s9d87s9d87s9d87s9d87s9d87s9d87s9d87s9d87s9d87sd7s9d87s9...',
                                margin: [0, 0, 0, 2],
                              },
              ...
              {
                text: 'INFORMACIÓN BANCARIA PARA PAGO',
                fontSize: 7,
                bold: true,
                decoration: 'underline',
                margin: [0, 0, 0, 2],
                    ...
                    [
                      {
                        stack: [
                          ...
                          {
                            text: 'Recibí Mercancía a mi entera satisfacción. Firma _____________________',
                            fontSize: 6,
                            alignment: 'right',
                            margin: [0, -10, 20, 10],
                          },

                          {
                            text: [
                              {
                                text: `Por el presente PAGARÉ me (nos) obligo (amos) a pagar en esta plaza el día `,
                                fontSize: 6,
                              },
                              {
                                text: `${fechaVencimientoStr} (AAAA-MM-DD)`,
                                fontSize: 6,
                                bold: true,
                              },
                              {
                                text: ` en moneda nacional a la orden de RODRIGO DANIEL ORTEGA CORREA la cantidad de `,
                                fontSize: 6,
                              },
                              {
                                text: `$${Number(datos.total).toFixed(2)}`,
                          ...
                          {
                            text: `A ${fechaEmisionStr} (AAAA-MM-DD HH:MM:SS)`,
                            fontSize: 6,
                            margin: [0, 5],
                          },
