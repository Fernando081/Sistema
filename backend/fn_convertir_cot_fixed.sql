DROP FUNCTION IF EXISTS public.fn_convertir_cotizacion_a_venta(integer);

CREATE OR REPLACE FUNCTION public.fn_convertir_cotizacion_a_venta(p_id_cotizacion integer, p_id_forma_pago integer, p_id_metodo_pago integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- 2. Construir el JSON de productos (Igual que antes)
    ...
    -- 3. Llamar a la función de venta
